...
export function MobileChatLayout({
  ...
  // Prevent hydration mismatch by tracking client-side mounting
  useEffect(() => {
    setIsClient(true);
  }, []);
  ...
  return (
    ...
    >
      {/* Mobile Header - only visible on small screens (client-side only to prevent hydration mismatch) */}
      {isClient && (
        <div 
      ...
      <div className="flex-1 overflow-hidden relative">
        {/* Desktop Layout - always rendered, visible on lg screens */}
        ...
        {/* Mobile Layout - only rendered on client to prevent hydration mismatch */}
        {isClient && (
        ...
        {/* Fallback for mobile on server - just show the desktop content */}
        {!isClient && (
          <div className="lg:hidden flex flex-col h-full">
            <div className="flex-1 flex flex-col min-w-0">
              {children}
      ...
      {/* Bottom Navigation - only on mobile, client-side only */}
      {isClient && (
      ...
      {/* Mobile Sidebar Overlay - client-side only */}
      {isClient && showMobileSidebar && (
